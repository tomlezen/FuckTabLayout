    ...
    private inner class SlidingTabIndicator internal constructor(context: Context) : LinearLayout(context) {
        ...
        private fun updateIndicatorPosition() {
            ...
            if (selectedPosition < childCount - 1) {
                (selectedTitle as FuckTabView).updateTextColor(getTextColorByFraction(1 - selectionOffset))
                (getChildAt(selectedPosition + 1) as FuckTabView).updateTextColor(getTextColorByFraction(selectionOffset))
            }
        ...
        override fun draw(canvas: Canvas) {
            ...
            if (indicatorLeft in 0..(indicatorRight - 1)) {
                val selectedIndicator = DrawableCompat.wrap(tabSelectedIndicator ?: defaultSelectionIndicator)
